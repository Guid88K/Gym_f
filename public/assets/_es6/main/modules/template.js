...
export default class Template {
    ...
    _uiInit() {
        ...
        this._lpageLoader           = jQuery('#pages-loader');
        this._lPage                 = jQuery('#pages-container');
        this._lSidebar              = jQuery('#sidebar');
        this._lSideOverlay          = jQuery('#side-overlay');
        this._lHeader               = jQuery('#pages-header');
        this._lHeaderSearch         = jQuery('#pages-header-search');
        this._lHeaderSearchInput    = jQuery('#pages-header-search-input');
        this._lHeaderLoader         = jQuery('#pages-header-loader');
        this._lMain                 = jQuery('#main-container');
        this._lFooter               = jQuery('#pages-footer');
    ...
    _uiHandleScroll() {
        ...
        // If .side-scroll is added to #pages-container enable custom scrolling
    ...
    _uiHandleMain() {
        ...
        if (self._lMain.length) {
            jQuery(window).on('resize.cb.main orientationchange.cb.main', e => {
                ...
                resizeTimeout = setTimeout(e => {
                    ...
                    if (self._lPage.hasClass('pages-header-fixed') || self._lPage.hasClass('pages-header-glass')) {
        ...
        // Add 'side-trans-enabled' class to #pages-container (enables sidebar and side overlay transition on open/close)
        // Fixes IE10, IE11 and Edge bug in which animation was executed on each pages load - really annoying!
    ...
    _uiHandleHeader() {
        ...
        if (self._lPage.hasClass('pages-header-glass') && self._lPage.hasClass('pages-header-fixed')) {
            jQuery(window).on('scroll.cb.header', e => {
                if (jQuery(e.currentTarget).scrollTop() > 60) {
                    self._lPage.addClass('pages-header-scroll');
                } else {
                    self._lPage.removeClass('pages-header-scroll');
                }
    ...
    _uiApiLayout(mode = 'init') {
        ...
        let layoutAPI = {
            init: () => {
                ...
                if (self._lPage.hasClass('enable-pages-overlay')) {
                    self._lPage.prepend('<div id="pages-overlay"></div>');

                    jQuery('#pages-overlay').on('click.cb.overlay', e => {
            ...
            header_fixed_toggle: () => {
                self._lPage.toggleClass('pages-header-fixed');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_fixed_on: () => {
                self._lPage.addClass('pages-header-fixed');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_fixed_off: () => {
                self._lPage.removeClass('pages-header-fixed');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_style_modern: () => {
                self._lPage.removeClass('pages-header-glass pages-header-inverse').addClass('pages-header-modern');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_style_classic: () => {
                self._lPage.removeClass('pages-header-glass pages-header-modern');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_style_glass: () => {
                self._lPage.removeClass('pages-header-modern').addClass('pages-header-glass');
                self._uiHandleHeader();
                self._uiHandleMain();
            },
            header_style_inverse_toggle: () => {
                if (!self._lPage.hasClass('pages-header-modern')) {
                    self._lPage.toggleClass('pages-header-inverse');
                }
            },
            header_style_inverse_on: () => {
                if (!self._lPage.hasClass('pages-header-modern')) {
                    self._lPage.addClass('pages-header-inverse');
                }
            },
            header_style_inverse_off: () => {
                if (!self._lPage.hasClass('pages-header-modern')) {
                    self._lPage.removeClass('pages-header-inverse');
                }
            },
            header_search_on: () => {
